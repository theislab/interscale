...
def create_spatial_cluster_distance(adata, gene, sample_col, distance_bins, bin_labels, threshold=95):
    ...
    assert len(bin_labels) + 1 == len(distance_bins), (
        "Number of bin labels must be equal to number of distance bins minus one"
    )
    ...
    for sample in adata.obs[sample_col].unique():
        ...
        zones = pd.cut(dists, bins=distance_bins, labels=bin_labels, include_lowest=True)
    ...
    adata.obs[f"{gene}_zone"] = pd.Categorical(
        adata.obs[f"{gene}_zone"], categories=bin_labels + ["No_Source", "Background"], ordered=True
    )

    return adata


def create_spatial_cluster_neighbours(adata, gene, sample_col, distance_bins=None, threshold=95):
    if distance_bins is None:
        distance_bins = [10, 20, 30]
    ...
    ctrl_test.obs["distance_cluster"] = np.digitize(
        ctrl_test.obs[f"dist_to_{gene}+"], bins=distance_bins
    )  # arbitrary cutoffs
