...
def split_adata_patient_stratified(
    adata: AnnData,
    patient_obs: str,
    stratify_obs: str,
    val_size: float = 0.1,
    ...
    if len(patient_stratify_df) != len(patient_stratify_df[patient_obs].unique()):
        raise ValueError(
            f"Patients have multiple {stratify_obs} labels. Each patient should have a single stratification label."
        )
    ...
    for label in stratify_labels:
        ...
        if len(label_patients) < 2:
            print(
                f"Warning: Only {len(label_patients)} patient(s) for stratification label '{label}'. All assigned to train."
            )
        ...
        if test_size > 0:
            ...
            if len(label_patients) * temp_size < 1:
                print(
                    f"Warning: Not enough patients for label '{label}' to create val/test sets. All assigned to train."
                )
                train_patients.extend(label_patients)
                continue

            train_temp, temp_patients = train_test_split(label_patients, test_size=temp_size, random_state=seed)

            # Split temp into val and test
            relative_val_size = val_size / temp_size
            val_temp, test_temp = train_test_split(temp_patients, test_size=1 - relative_val_size, random_state=seed)
            ...
            train_temp, val_temp = train_test_split(label_patients, test_size=val_size, random_state=seed)
