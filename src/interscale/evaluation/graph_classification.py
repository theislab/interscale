...
def calculate_pr_auc(result: AnnData, plot_curve: bool = True):
...
def pr_auc_curve(y_true_binary: np.ndarray, y_pred: np.ndarray, classes: list[str]):
...
def plot_adata_grouped_heatmaps(
    ...
    **kwargs
    ...
    ...     adata_dict=adata_dict, group_by="CellType", save_path="output/cls_heatmap.png"
    ... )
    ...
    if labels is None:
        ...
        if len(labels) != len(adata_dict):
            raise ValueError(
                f"Number of labels ({len(labels)}) must match number of AnnData objects ({len(adata_dict)})"
            )
