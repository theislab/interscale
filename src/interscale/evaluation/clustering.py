...
def leiden_cluster_embeddings(result, seeds=[0, 1, 2, 3, 4], leiden_res=1.0):
    ...
    for seed in seeds:
        sc.pp.neighbors(
            result, n_neighbors=40, use_rep="combined_local_emb", key_added=f"local_{seed}", random_state=seed
        )
        sc.pp.neighbors(
            result, n_neighbors=40, use_rep="combined_global_emb", key_added=f"global_{seed}", random_state=seed
        )

        sc.tl.leiden(
            result,
            resolution=leiden_res,
            neighbors_key=f"local_{seed}",
            key_added=f"leiden_local_{seed}",
            random_state=seed,
        )
        sc.tl.leiden(
            result,
            resolution=leiden_res,
            neighbors_key=f"global_{seed}",
            key_added=f"leiden_global_{seed}",
            random_state=seed,
        )
