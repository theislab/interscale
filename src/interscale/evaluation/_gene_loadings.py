...
def gene_loadings(
    ...
    if n_genes_l != n_genes_g:
        raise ValueError(f"Gene count mismatch between decoders: local has {n_genes_l}, global has {n_genes_g}")
    ...
    if Zl.shape[1] != n_latent_l:
        raise ValueError(
            f"Latent dimension mismatch: local decoder has {n_latent_l}, but {local_latent_key} has {Zl.shape[1]}"
        )
    if Zg.shape[1] != n_latent_g:
        raise ValueError(
            f"Latent dimension mismatch: global decoder has {n_latent_g}, but {global_latent_key} has {Zg.shape[1]}"
        )
    ...
    if len(x_std) != n_genes_l:
        raise ValueError(f"Gene count mismatch: decoders have {n_genes_l} genes, expression layer has {len(x_std)}")
