...
from typing import TYPE_CHECKING, Any, Literal, NamedTuple, Optional, Union  # Added Literal here
...
def sliding_window(
    ...
    for i, lib in enumerate(libraries):
        ...
        if partial_window == "merge":
            ...
            partial_windows = {idx: cells for idx, cells in window_assignments.items() 
                            if len(cells) < median_cells * 0.5}  # threshold at 50% of median
            ...
            for p_idx, p_cells in partial_windows.items():
                ...
                p_center = np.array([(p_window["x_start"] + p_window["x_end"])/2,
                                   (p_window["y_start"] + p_window["y_end"])/2])
                ...
                for idx, window in windows.iterrows():
                    if idx not in partial_windows:
                        center = np.array([(window["x_start"] + window["x_end"])/2,
                                         (window["y_start"] + window["y_end"])/2])
