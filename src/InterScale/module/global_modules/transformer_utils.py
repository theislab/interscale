...
from torch.nn.functional import _canonical_mask, softmax, dropout, linear, pad, _none_or_dtype, _mha_shape_check, _in_projection_packed, _in_projection, scaled_dot_product_attention
...
__all__ = ['multi_head_attention_forward_with_gradients', 'TransformerEncoderLayerWithSpecialMultiHead']
...
def multi_head_attention_forward_with_gradients(
    ...
    if has_torch_function(tens_ops):
        return handle_torch_function(
            multi_head_attention_forward_with_gradients,
            tens_ops,
            query,
            key,
            value,
    ...
    if need_weights:
        ...
        print(f"LOGIT STATS: MeanAbs: {abs_mean:.6f} | Max: {max_val:.6f}")
