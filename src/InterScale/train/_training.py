        ...
        **kwargs
        ...
        if self._cfg.model.save is not None:
            ...
            if 'classification' in self._cfg.dataset.prediction_task:
                checkpoint_callback = ModelCheckpoint(dirpath=self._cfg.model.save, filename=run_name, monitor="val_accuracy", mode="max", ) # save model if validation accuracy increases
