...
# --- Model type from command line argument (default: CombinedModel) ---
MODEL_TYPE=${1:-"CombinedModel"}

TIMESTAMP=$(date +%Y-%m-%d_%H-%M-%S)
LOG_FILE="logs/${TIMESTAMP}_${SLURM_JOB_ID}_${MODEL_TYPE}.out"

mkdir -p logs
exec > >(tee "${LOG_FILE}") 2>&1

echo "Running sweep for model type: ${MODEL_TYPE}"
